...
public class NeuronLayer
{
	...
	public void connectTo(NeuronLayer connection, double[] weights)
	{
		...
		if(weights == null)
		{
			...
			weightMods = new double[connection.size()*size()];
	...
	public void recieveWeightModification(double[] mods)
	{
		for(int i = 0; i < weightMods.length; i++)
			weightMods[i] += mods[i];
	}
		...
		if(connection != null)
		{
			...
			for(int i = 0; i < weights.length; i++)
			{
				...
				if(Math.random()<changeRate)
				{
					if(Math.random() < .75)
					{
						newWeights[i] = newWeights[i] + (Math.random()<.5?0.1:-0.1);
					}
					else
						newWeights[i] = Math.random()-.5;
				}
			}
	...
	public void iterate()
	{
		if(connection != null)
		{
			for(int i = 0; i < weights.length; i++)
			{
				...
				{
					case BOOLEAN_ACTIVATION:
						connection.add(i%connection.size(), (weights[i]+weightMods[i])*booleanActivation(activations[i/connection.size()]));
						break;
					case LINIAR_ACTIVATION:
						connection.add(i%connection.size(), (weights[i]+weightMods[i])*liniarActivation(activations[i/connection.size()]));
						break;
					case SIGMOID_ACTIVATION:
						connection.add(i%connection.size(), (weights[i]+weightMods[i])*sigmoidActivation(activations[i/connection.size()]));
						break;
					case HYPERBOLIC_TANGENT_ACTIVATION:
						connection.add(i%connection.size(), (weights[i]+weightMods[i])*hyperbolicTangentActivation(activations[i/connection.size()]));
						break;
				}
			}
			clear();
